...
function _M.write_to(self, buf)
	return brz_w.write_msg_without_type(buf, function(fbuf)
        ...
        if myMap_size > 0 then
            brz_w.write_packed_map_field(fbuf, 3, myMap_size, function(fbuf)
        ...
        if myArray_size > 0 then
            brz_w.write_packed_array_field(fbuf, 4, myArray_size, function(fbuf)
        ...
        if enumArray_size > 0 then
            brz_w.write_packed_array_field(fbuf, 7, enumArray_size, function(fbuf)
                
                local mt_v = self.enumArray[1]
                brz_w.write_message_type(fbuf, mt_v:get_name())
